...
elif [[ $certificate = '/root/ygkkkca/cert.crt' && -n $(curl -s6m6 ip.p3terx.com -k | sed -n 1p) ]]; then
sed -i "2s/$oldserver/\[${ymip}\]/g" /root/HY/acl/v2rayn.json
sed -i "s/$oldserver/\[${ymip}\]/" /root/HY/URL.txt
sed -i "23s/$oldserver/${ymip}/g" /root/HY/acl/Cmeta-hy.yaml
elif [[ $certificate = '/root/ygkkkca/cert.crt' && -z $(curl -s6m6 ip.p3terx.com -k | sed -n 1p) ]]; then
sed -i "2s/$oldserver/${ymip}/g" /root/HY/acl/v2rayn.json
sed -i "s/$oldserver/${ymip}/" /root/HY/URL.txt
sed -i "23s/$oldserver/${ymip}/g" /root/HY/acl/Cmeta-hy.yaml
